...
- *(engine)* make the R4 max-hold panic opt-in via EngineCfg.max_hold_panic

### Other

- add docs.rs metadata to dsl / compile / cli crates
...
- add DispatchOutcome::Skip tier + BP DSL skip_on sugar + structural FlowEval error (GH #76)
...
- *(server)* add POST /v1/runs/:id/rerun-from — GH #71 Layer A

### Other

- *(cli)* explain Operator naming glue in guide + all bp scaffolds
