...
| tool | purpose | side effect |
|---|---|---|
| `mse_operator_join` | Join as an Operator session: `POST /v1/operators` (mint `sid`+token, pin `capability_manifest`, record the `desc` line that identifies this session) then connect `WS /v1/operators/:sid/ws` with the Bearer token (kept process-local, never returned). Returns `{sid}`. Params: `capability_manifest?`, `desc` (**required here**). `desc` is ~50 characters saying what you are touching and what you are doing to it — the thing a later reader tells your session apart by on `mse_operator_list`; do not write the repo path, worktree path, Run id, goal or start time, which are recorded for you at each assignment. The HTTP route itself accepts a join without one (join must never be gated, or a handover could be locked out), so the insistence lives here, where the caller is an AI that already knows the answer. The manifest is optional: Runner-backed launches proceed declaration-only without one (the operator self-checks); a manifest entry matching each launch variant is required only when the Blueprint sets `strict_binding = true`, which also requires the launch to carry `operator_sid` — a manifest is attested through the pinned session and through nothing else. A join claims no role and can never conflict: which Run you drive is decided per Run, by launching it (`swarm_run` pins this session) or by taking a seat on one (`POST /v1/runs/:id/acquire`), so two drivers joining one server is the ordinary case. | Mutating — opens a WS session; requires `mse serve` reachable. |
