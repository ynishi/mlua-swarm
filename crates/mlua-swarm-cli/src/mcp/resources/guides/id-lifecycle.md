...
verify  Engine::verify_token — 4 steps: (1) constant-time signature check,
        (2) expiry — skipped for Role::Operator, whose token never leaves
        the process, so the TTL bounded no reachable capability; Worker /
        Senior / Observer keep it, (3) role × verb gate, (4) server-side
        uses_left consume, looked up by CapToken::fingerprint (SHA-256 of the
...
expire  expire_at passes or max_uses exhausts; worker handles die with the
        server process (in-memory map). Operator session tokens are the
        exception: they still carry an expire_at, but verification does
        not gate on it and their max_uses is None, so they stay usable
